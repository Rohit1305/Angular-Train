import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css']
})
export class AppComponent implements OnInit{
  count=0;
  title = '01-first-project';
  name='Rohit';
  inputValue=''
  inputText='';
  submittedText='';
  username='';

  imgurl="data:image/png;base64,"
  linkUrl="http://www.google.com";

  message: string = '';
  counter=0
  incrementCounter() {
    if (this.counter < 10) {
      this.counter++;
    }
  }
  decrementCounter() {
    if (this.counter > 0) {
      this.counter--;
    }
  }
  updateInput(val:any){
    this.inputValue=val;
    
  }
  showMessage(buttonType: string) {
    this.message = `You clicked on ${buttonType} button.`;
  }

  ngOnInit(): void {
    setTimeout(() => {
      this.count++;
      if(this.count%2==0){
        this.imgurl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_Nli59H579Dqcn4BmenqXTNWpYm8f-ZHUAg&usqp=CAU";
      }else{
        this.imgurl="https://external-preview.redd.it/podcast-how-the-creator-of-angular-is-dehydrating-the-web-v0-s8WeIc29KlNN-PxSF1bd6RN5JUz_5cr8VeSLYGsY900.jpg?auto=webp&s=74de2160606358a26e0e4751c190c84ee370b982";
      }
    }, 2000); 
  }

  showPrint() {
    this.submittedText =  this.inputText;
    return this.submittedText;
  }
  showPrintWithBox(value: string) {
    this.submittedText = 'You submitted: ' + value;
  }

  clearInputField() {
    this.username = '';
  }
  onUpdateValue(event : Event){ 
    console.log(event); 
    this.inputText=(<HTMLInputElement>event.target).value; 
  }

}
